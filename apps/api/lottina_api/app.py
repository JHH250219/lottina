from flask import Flask, render_template, request, jsonify, redirect, url_for, flash, send_from_directory, abort, send_file
...
def inject_globals():
    return {
        ...
        "current_user": current_user,  # später Authentifizierung einbauen
    }
...
def index():
    ...
    events = (db.session.query(Offer)
              .order_by(Offer.dt_start.asc().nulls_last(), Offer.id.desc())
              .limit(9)
              .all())

    testimonials = [
        {"name": "Lena & Tom", "text": "Endlich alles an einem Ort – mega praktisch!", "img": "img/oma-paper-512.png"},
        {"name": "Mara",       "text": "Hab so viele neue Kinderkurse entdeckt.",       "img": "img/oma-cookies.png"},
        {"name": "Philipp",    "text": "Die Karte ist Gold wert.",                      "img": "img/lottina_logo.png"},
    ]
...
def results():
    ...
    qry = db.session.query(Offer).join(Location, Offer.location_id == Location.id, isouter=True)
    ...
    if day_start and day_end:
        qry = qry.filter(and_(Offer.dt_start >= day_start, Offer.dt_start < day_end))
    ...
    if request.args.get("always") == "1":
        qry = qry.filter(Offer.dt_start.is_(None), Offer.dt_end.is_(None))
    ...
    categories = [name for (name,) in db.session.query(Category.name)
                  .distinct()
...
def notify():
    ...
    # TODO: Später hier persistieren (DB/S3) oder an Maildienst/Webhook senden.
...
def login():
    if request.method == "POST":
        ...
        if user and user.check_password(password):
            login_user(user)  # Session setzen
            flash("Willkommen zurück!", "success")
            return redirect(url_for("dashboard"))  # passe Ziel an
        flash("Ungültige Anmeldedaten.", "danger")
    return render_template("login.html")  # dein vorhandenes Template
...
def register():
    if request.method == "POST":
        ...
        # sehr einfache Validierung – gern verschärfen
...
def _serve_uploaded_image(fname):
    """
    Sicheres Ausliefern hochgeladener Bilder aus IMAGE_FOLDER.
    Verhindert Path Traversal und liefert 404, wenn die Datei fehlt.
    """
    # optional härten
    safe_name = secure_filename(fname)
    # Wir erlauben Subfolder (wegen <path:...>), daher nicht einfach nur secure_filename nutzen.
    # Stattdessen: realpath prüfen, dass es unter IMAGE_FOLDER liegt:
...
def ocr_upload():
    ...
    # robustes Einlesen (Unicode-Pfad freundlich)
    img_array = None
    try:
        # cv2.imread hat manchmal Probleme mit Nicht-ASCII Pfaden → np.fromfile + imdecode
...
def create_event():
    ...
    if date_s:
        try:
            ...
            dt_start = datetime.strptime(f"{date_s} {start_token}", "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
        except Exception:
            dt_start = None
        if time_end_s:
            try:
                dt_end_candidate = datetime.strptime(f"{date_s} {time_end_s}", "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
    ...
    lat = _to_float(f.get("lat")); lon = _to_float(f.get("lon"))

    if not location_name_raw or len(location_name_raw) > MAX_LOC_NAME:
        addr_guess, city_guess = extract_addr_city_from_text(f.get("description") or "")
    ...
    summary = shorten(" · ".join([part for part in summary_parts if part]), MAX_SUMMARY_LEN) if summary_parts else None
    ...
    # --- Kategorie anhängen (relational) ---
    cat_name = (f.get("category") or "").strip()
    if cat_name:
        _AUML = {"ä":"ae","ö":"oe","ü":"ue","ß":"ss"}
        def slugify(s: str) -> str:
            s = s.strip().lower()
            for a,b in _AUML.items(): s = s.replace(a,b)
    ...
    try:
        ...
        # → nach dem Speichern auf die Ergebnisseite
        return redirect(url_for("suchergebnisse"))
        # Optional: direkt zur Detailseite
        # return redirect(url_for("event_detail", slug_or_id=str(offer.id)))
    except IntegrityError:
        db.session.rollback()
